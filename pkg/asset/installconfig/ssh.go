...
type sshPublicKey struct{ Key string }
...
func (a *sshPublicKey) Generate(asset.Parents) error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	pubKeys := map[string]string{noSSHKey: ""}
	home := os.Getenv("HOME")
	if home != "" {
	...
	sort.Strings(paths)
	var path string
	if err := survey.AskOne(&survey.Select{Message: "SSH Public Key", Help: "The SSH public key used to access all nodes within the cluster. This is optional.", Options: paths, Default: noSSHKey}, &path, func(ans interface{}) error {
		choice := ans.(string)
