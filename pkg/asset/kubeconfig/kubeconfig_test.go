...
func TestKubeconfigGenerate(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	rootCA := &testCertKey{key: "THIS IS ROOT CA KEY DATA", cert: "THIS IS ROOT CA CERT DATA"}
	adminCert := &testCertKey{key: "THIS IS ADMIN KEY DATA", cert: "THIS IS ADMIN CERT DATA"}
	kubeletCert := &testCertKey{key: "THIS IS KUBELET KEY DATA", cert: "THIS IS KUBELET CERT DATA"}
	installConfig := &types.InstallConfig{ObjectMeta: metav1.ObjectMeta{Name: "test-cluster-name"}, BaseDomain: "test.example.com"}
	tests := []struct {
		name		string
		userName	string
		filename	string
		...
		apiURL		string
		expectedData	[]byte
	}{{name: "admin kubeconfig", userName: "admin", filename: "auth/kubeconfig", clientCert: adminCert, apiURL: "https://api-int.test-cluster-name.test.example.com:6443", expectedData: []byte(`clusters:
- cluster:
...
`)}, {name: "kubelet kubeconfig", userName: "kubelet", filename: "auth/kubeconfig-kubelet", clientCert: kubeletCert, apiURL: "https://api-int.test-cluster-name.test.example.com:6443", expectedData: []byte(`clusters:
- cluster:
    certificate-authority-data: VEhJUyBJUyBST09UIENBIENFUlQgREFUQQ==
    server: https://api-int.test-cluster-name.test.example.com:6443
  name: test-cluster-name
