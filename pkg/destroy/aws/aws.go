...
func (o *ClusterUninstaller) Run() error {
	...
	awsSession.Handlers.Build.PushBackNamed(request.NamedHandler{Name: "openshiftInstaller.OpenshiftInstallerUserAgentHandler", Fn: request.MakeAddToUserAgentHandler("OpenShift/4.x Destroyer", version.Raw)})
	tagClients := []*resourcegroupstaggingapi.ResourceGroupsTaggingAPI{resourcegroupstaggingapi.New(awsSession)}
	tagClientNames := map[*resourcegroupstaggingapi.ResourceGroupsTaggingAPI]string{tagClients[0]: o.Region}
	if o.Region != "us-east-1" {
		tagClient := resourcegroupstaggingapi.New(awsSession, aws.NewConfig().WithRegion("us-east-1"))
		tagClients = append(tagClients, tagClient)
		tagClientNames[tagClient] = "us-east-1"
	}
	deleted := map[string]struct{}{}
	iamClient := iam.New(awsSession)
	iamRoleSearch := &iamRoleSearch{client: iamClient, filters: o.Filters, logger: o.Logger}
	iamUserSearch := &iamUserSearch{client: iamClient, filters: o.Filters, logger: o.Logger}
	err = wait.PollImmediateInfinite(time.Second*10, func() (done bool, err error) {
		var loopError error
		...
		for _, tagClient := range tagClients {
			...
			for _, filter := range o.Filters {
				...
				for key, value := range filter {
					tagFilters = append(tagFilters, &resourcegroupstaggingapi.TagFilter{Key: aws.String(key), Values: []*string{aws.String(value)}})
				}
				err = tagClient.GetResourcesPages(&resourcegroupstaggingapi.GetResourcesInput{TagFilters: tagFilters}, func(results *resourcegroupstaggingapi.GetResourcesOutput, lastPage bool) bool {
...
func (search *iamRoleSearch) arns() ([]string, error) {
	...
	arns := []string{}
	var lastError error
	err := search.client.ListRolesPages(&iam.ListRolesInput{}, func(results *iam.ListRolesOutput, lastPage bool) bool {
...
func (search *iamUserSearch) arns() ([]string, error) {
	...
	arns := []string{}
	var lastError error
	err := search.client.ListUsersPages(&iam.ListUsersInput{}, func(results *iam.ListUsersOutput, lastPage bool) bool {
...
func deleteEC2Image(client *ec2.EC2, id string, filter Filter, logger logrus.FieldLogger) error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	response, err := client.DescribeImages(&ec2.DescribeImagesInput{ImageIds: []*string{&id}})
	if err != nil {
	...
	_, err = client.CreateTags(&ec2.CreateTagsInput{Resources: snapshots, Tags: tagsForFilter(filter)})
	if err != nil {
		err = errors.Wrapf(err, "tagging snapshots for %s", id)
	}
	_, err = client.DeregisterImage(&ec2.DeregisterImageInput{ImageId: &id})
	if err != nil {
		if err.(awserr.Error).Code() == "InvalidAMIID.NotFound" {
			return nil
		}
...
func deleteEC2Instance(ec2Client *ec2.EC2, iamClient *iam.IAM, id string, logger logrus.FieldLogger) error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	response, err := ec2Client.DescribeInstances(&ec2.DescribeInstancesInput{InstanceIds: []*string{aws.String(id)}})
	if err != nil {
	...
	for _, reservation := range response.Reservations {
		for _, instance := range reservation.Instances {
			...
			_, err := ec2Client.TerminateInstances(&ec2.TerminateInstancesInput{InstanceIds: []*string{instance.InstanceId}})
			if err != nil {
				return err
			}
			logger.Info("Deleted")
...
func deleteEC2InternetGateway(client *ec2.EC2, id string, logger logrus.FieldLogger) error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	response, err := client.DescribeInternetGateways(&ec2.DescribeInternetGatewaysInput{InternetGatewayIds: []*string{aws.String(id)}})
	if err != nil {
		return err
	}
	for _, gateway := range response.InternetGateways {
		for _, vpc := range gateway.Attachments {
			...
			_, err := client.DetachInternetGateway(&ec2.DetachInternetGatewayInput{InternetGatewayId: gateway.InternetGatewayId, VpcId: vpc.VpcId})
			if err == nil {
	...
	_, err = client.DeleteInternetGateway(&ec2.DeleteInternetGatewayInput{InternetGatewayId: &id})
	if err != nil {
		return err
	}
	logger.Info("Deleted")
...
func deleteEC2NATGatewaysByVPC(client *ec2.EC2, vpc string, logger logrus.FieldLogger) error {
	...
	defer _logClusterCodePath()
	var lastError error
	err := client.DescribeNatGatewaysPages(&ec2.DescribeNatGatewaysInput{Filter: []*ec2.Filter{{Name: aws.String("vpc-id"), Values: []*string{&vpc}}}}, func(results *ec2.DescribeNatGatewaysOutput, lastPage bool) bool {
...
func deleteEC2RouteTableObject(client *ec2.EC2, table *ec2.RouteTable, logger logrus.FieldLogger) error {
	...
	_, err := client.DeleteRouteTable(&ec2.DeleteRouteTableInput{RouteTableId: table.RouteTableId})
	if err != nil {
		return err
	}
	logger.Info("Deleted")
...
func deleteEC2SecurityGroup(client *ec2.EC2, id string, logger logrus.FieldLogger) error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	response, err := client.DescribeSecurityGroups(&ec2.DescribeSecurityGroupsInput{GroupIds: []*string{aws.String(id)}})
	if err != nil {
	...
	for _, group := range response.SecurityGroups {
		if len(group.IpPermissions) > 0 {
			_, err := client.RevokeSecurityGroupIngress(&ec2.RevokeSecurityGroupIngressInput{GroupId: group.GroupId, IpPermissions: group.IpPermissions})
			if err != nil {
		...
		if len(group.IpPermissionsEgress) > 0 {
			_, err := client.RevokeSecurityGroupEgress(&ec2.RevokeSecurityGroupEgressInput{GroupId: group.GroupId, IpPermissions: group.IpPermissionsEgress})
			if err != nil {
	...
	_, err = client.DeleteSecurityGroup(&ec2.DeleteSecurityGroupInput{GroupId: aws.String(id)})
	if err != nil {
		if err.(awserr.Error).Code() == "InvalidGroup.NotFound" {
			return nil
		}
...
func deleteEC2NetworkInterfaceByVPC(client *ec2.EC2, vpc string, logger logrus.FieldLogger) error {
	...
	defer _logClusterCodePath()
	var lastError error
	err := client.DescribeNetworkInterfacesPages(&ec2.DescribeNetworkInterfacesInput{Filters: []*ec2.Filter{{Name: aws.String("vpc-id"), Values: []*string{&vpc}}}}, func(results *ec2.DescribeNetworkInterfacesOutput, lastPage bool) bool {
...
func deleteEC2VPC(ec2Client *ec2.EC2, elbClient *elb.ELB, elbv2Client *elbv2.ELBV2, id string, logger logrus.FieldLogger) error {
	...
	for _, helper := range [](func(client *ec2.EC2, vpc string, logger logrus.FieldLogger) error){deleteEC2NATGatewaysByVPC, deleteEC2NetworkInterfaceByVPC, deleteEC2RouteTablesByVPC, deleteEC2VPCEndpointsByVPC} {
		err := helper(ec2Client, id, logger)
		if err != nil {
			return err
		}
	}
	_, err := ec2Client.DeleteVpc(&ec2.DeleteVpcInput{VpcId: aws.String(id)})
	if err != nil {
		return err
	}
	logger.Info("Deleted")
...
func deleteEC2VPCEndpointsByVPC(client *ec2.EC2, vpc string, logger logrus.FieldLogger) error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	response, err := client.DescribeVpcEndpoints(&ec2.DescribeVpcEndpointsInput{Filters: []*ec2.Filter{{Name: aws.String("vpc-id"), Values: []*string{aws.String(vpc)}}}})
	if err != nil {
		return err
	}
...
func deleteElasticLoadBalancerClassicByVPC(client *elb.ELB, vpc string, logger logrus.FieldLogger) error {
	...
	defer _logClusterCodePath()
	var lastError error
	err := client.DescribeLoadBalancersPages(&elb.DescribeLoadBalancersInput{}, func(results *elb.DescribeLoadBalancersOutput, lastPage bool) bool {
...
func deleteElasticLoadBalancerTargetGroupsByVPC(client *elbv2.ELBV2, vpc string, logger logrus.FieldLogger) error {
	...
	defer _logClusterCodePath()
	var lastError error
	err := client.DescribeTargetGroupsPages(&elbv2.DescribeTargetGroupsInput{}, func(results *elbv2.DescribeTargetGroupsOutput, lastPage bool) bool {
...
func deleteElasticLoadBalancerV2ByVPC(client *elbv2.ELBV2, vpc string, logger logrus.FieldLogger) error {
	...
	defer _logClusterCodePath()
	var lastError error
	err := client.DescribeLoadBalancersPages(&elbv2.DescribeLoadBalancersInput{}, func(results *elbv2.DescribeLoadBalancersOutput, lastPage bool) bool {
...
func deleteIAMInstanceProfile(client *iam.IAM, profileARN arn.ARN, logger logrus.FieldLogger) error {
	...
	response, err := client.GetInstanceProfile(&iam.GetInstanceProfileInput{InstanceProfileName: &name})
	if err != nil {
		if err.(awserr.Error).Code() == iam.ErrCodeNoSuchEntityException {
			return nil
		}
	...
	for _, role := range profile.Roles {
		_, err = client.RemoveRoleFromInstanceProfile(&iam.RemoveRoleFromInstanceProfileInput{InstanceProfileName: profile.InstanceProfileName, RoleName: role.RoleName})
		if err != nil {
...
func deleteIAMRole(client *iam.IAM, roleARN arn.ARN, logger logrus.FieldLogger) error {
	...
	var lastError error
	err = client.ListRolePoliciesPages(&iam.ListRolePoliciesInput{RoleName: &name}, func(results *iam.ListRolePoliciesOutput, lastPage bool) bool {
		for _, policy := range results.PolicyNames {
			_, err := client.DeleteRolePolicy(&iam.DeleteRolePolicyInput{RoleName: &name, PolicyName: policy})
			if err != nil {
				if lastError != nil {
					logger.Debug(lastError)
				}
	...
	err = client.ListInstanceProfilesForRolePages(&iam.ListInstanceProfilesForRoleInput{RoleName: &name}, func(results *iam.ListInstanceProfilesForRoleOutput, lastPage bool) bool {
...
func deleteIAMUser(client *iam.IAM, id string, logger logrus.FieldLogger) error {
	...
	err := client.ListUserPoliciesPages(&iam.ListUserPoliciesInput{UserName: &id}, func(results *iam.ListUserPoliciesOutput, lastPage bool) bool {
		for _, policy := range results.PolicyNames {
			_, err := client.DeleteUserPolicy(&iam.DeleteUserPolicyInput{UserName: &id, PolicyName: policy})
			if err != nil {
				if lastError != nil {
					logger.Debug(lastError)
				}
	...
	err = client.ListAccessKeysPages(&iam.ListAccessKeysInput{UserName: &id}, func(results *iam.ListAccessKeysOutput, lastPage bool) bool {
		for _, key := range results.AccessKeyMetadata {
			_, err := client.DeleteAccessKey(&iam.DeleteAccessKeyInput{UserName: &id, AccessKeyId: key.AccessKeyId})
			if err != nil {
				if lastError != nil {
					logger.Debug(lastError)
				}
	...
	_, err = client.DeleteUser(&iam.DeleteUserInput{UserName: &id})
	if err != nil {
		return err
	}
	logger.Info("Deleted")
...
func deleteRoute53(session *session.Session, arn arn.ARN, logger logrus.FieldLogger) error {
	...
	if len(sharedZoneID) != 0 {
		err = client.ListResourceRecordSetsPages(&route53.ListResourceRecordSetsInput{HostedZoneId: aws.String(sharedZoneID)}, func(results *route53.ListResourceRecordSetsOutput, lastPage bool) bool {
	...
	var lastError error
	err = client.ListResourceRecordSetsPages(&route53.ListResourceRecordSetsInput{HostedZoneId: aws.String(id)}, func(results *route53.ListResourceRecordSetsOutput, lastPage bool) bool {
		for _, recordSet := range results.ResourceRecordSets {
	...
	_, err = client.DeleteHostedZone(&route53.DeleteHostedZoneInput{Id: aws.String(id)})
	if err != nil {
		return err
	}
	logger.Info("Deleted")
...
func deleteS3(session *session.Session, arn arn.ARN, logger logrus.FieldLogger) error {
	...
	iter := s3manager.NewDeleteListIterator(client, &s3.ListObjectsInput{Bucket: aws.String(arn.Resource)})
	err := s3manager.NewBatchDeleteWithClient(client).Delete(aws.BackgroundContext(), iter)
	if err != nil {
		return err
	}
	logger.Debug("Emptied")
	_, err = client.DeleteBucket(&s3.DeleteBucketInput{Bucket: aws.String(arn.Resource)})
	if err != nil {
		return err
	}
	logger.Info("Deleted")
