...
func TestSetPlatformDefaults(t *testing.T) {
	...
	}{{name: "empty", platform: &libvirt.Platform{}, expected: defaultPlatform()}, {name: "URI present", platform: &libvirt.Platform{URI: "test-uri"}, expected: func() *libvirt.Platform {
		p := defaultPlatform()
		p.URI = "test-uri"
		return p
	}()}, {name: "Network present", platform: &libvirt.Platform{Network: func() *libvirt.Network {
		n := &libvirt.Network{}
		SetNetworkDefaults(n)
		n.IfName = "test-if"
		return n
