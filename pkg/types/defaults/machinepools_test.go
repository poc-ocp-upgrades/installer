...
func TestSetMahcinePoolDefaults(t *testing.T) {
	...
	cases := []struct {
	...
	}{{name: "empty", pool: &types.MachinePool{}, expected: defaultMachinePool("")}, {name: "default", pool: defaultMachinePool("test-name"), expected: defaultMachinePool("test-name")}, {name: "non-default replicas", pool: func() *types.MachinePool {
		...
		p.Replicas = pointer.Int64Ptr(5)
		return p
	}()}, {name: "libvirt replicas", pool: &types.MachinePool{}, platform: "libvirt", expected: func() *types.MachinePool {
		p := defaultMachinePool("")
		p.Replicas = pointer.Int64Ptr(1)
		return p
	}()}, {name: "non-default hyperthreading", pool: func() *types.MachinePool {
		p := defaultMachinePool("test-name")
