...
func TestSetInstallConfigDefaults(t *testing.T) {
	...
	}{{name: "empty", config: &types.InstallConfig{}, expected: defaultInstallConfig()}, {name: "empty AWS", config: &types.InstallConfig{Platform: types.Platform{AWS: &aws.Platform{}}}, expected: defaultAWSInstallConfig()}, {name: "empty Azure", config: &types.InstallConfig{Platform: types.Platform{Azure: &azure.Platform{}}}, expected: defaultAzureInstallConfig()}, {name: "empty Libvirt", config: &types.InstallConfig{Platform: types.Platform{Libvirt: &libvirt.Platform{}}}, expected: defaultLibvirtInstallConfig()}, {name: "empty OpenStack", config: &types.InstallConfig{Platform: types.Platform{OpenStack: &openstack.Platform{}}}, expected: defaultOpenStackInstallConfig()}, {name: "Networking present", config: &types.InstallConfig{Networking: &types.Networking{}}, expected: defaultInstallConfig()}, {name: "Networking types present", config: &types.InstallConfig{Networking: &types.Networking{NetworkType: "test-networking-type"}}, expected: func() *types.InstallConfig {
		c := defaultInstallConfig()
		c.Networking.NetworkType = "test-networking-type"
		return c
	}()}, {name: "Service network present", config: &types.InstallConfig{Networking: &types.Networking{ServiceNetwork: []ipnet.IPNet{*ipnet.MustParseCIDR("1.2.3.4/8")}}}, expected: func() *types.InstallConfig {
		c := defaultInstallConfig()
		c.Networking.ServiceNetwork[0] = *ipnet.MustParseCIDR("1.2.3.4/8")
		return c
	}()}, {name: "Cluster network present", config: &types.InstallConfig{Networking: &types.Networking{ClusterNetwork: []types.ClusterNetworkEntry{{CIDR: *ipnet.MustParseCIDR("8.8.0.0/18"), HostPrefix: 22}}}}, expected: func() *types.InstallConfig {
		c := defaultInstallConfig()
		c.Networking.ClusterNetwork = []types.ClusterNetworkEntry{{CIDR: *ipnet.MustParseCIDR("8.8.0.0/18"), HostPrefix: 22}}
		return c
	}()}, {name: "control plane present", config: &types.InstallConfig{ControlPlane: &types.MachinePool{}}, expected: defaultInstallConfig()}, {name: "Compute present", config: &types.InstallConfig{Compute: []types.MachinePool{{Name: "test-compute"}}}, expected: func() *types.InstallConfig {
		c := defaultInstallConfig()
		c.Compute = []types.MachinePool{*defaultMachinePool("test-compute")}
		return c
	}()}, {name: "AWS platform present", config: &types.InstallConfig{Platform: types.Platform{AWS: &aws.Platform{}}}, expected: func() *types.InstallConfig {
		c := defaultAWSInstallConfig()
		return c
	}()}, {name: "Azure platform present", config: &types.InstallConfig{Platform: types.Platform{Azure: &azure.Platform{}}}, expected: func() *types.InstallConfig {
		c := defaultAzureInstallConfig()
		return c
	}()}, {name: "Libvirt platform present", config: &types.InstallConfig{Platform: types.Platform{Libvirt: &libvirt.Platform{URI: "test-uri"}}}, expected: func() *types.InstallConfig {
		c := defaultLibvirtInstallConfig()
		c.Platform.Libvirt.URI = "test-uri"
		return c
	}()}, {name: "OpenStack platform present", config: &types.InstallConfig{Platform: types.Platform{OpenStack: &openstack.Platform{}}}, expected: func() *types.InstallConfig {
		c := defaultOpenStackInstallConfig()
		return c
	}()}, {name: "None platform present", config: &types.InstallConfig{Platform: types.Platform{None: &none.Platform{}}}, expected: func() *types.InstallConfig {
