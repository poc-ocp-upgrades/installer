...
func newDestroyClusterCmd() *cobra.Command {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &cobra.Command{Use: "cluster", Short: "Destroy an OpenShift cluster", Args: cobra.ExactArgs(0), Run: func(_ *cobra.Command, _ []string) {
...
func newDestroyBootstrapCmd() *cobra.Command {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &cobra.Command{Use: "bootstrap", Short: "Destroy the bootstrap resources", Args: cobra.ExactArgs(0), Run: func(cmd *cobra.Command, args []string) {
		cleanup := setupFileHook(rootOpts.dir)
		defer cleanup()
